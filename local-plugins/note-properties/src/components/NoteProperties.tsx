...
function renderSourceValue(value: unknown, ctx: RenderCtx): preact.JSX.Element | string {
...
function renderChipList(key: string, values: unknown[], ctx: RenderCtx): preact.JSX.Element {
  const items = values.map((item, idx) => {
    if (key === "tags" && (typeof item === "string" || typeof item === "number")) {
      ...
      return (
        <a key={idx} href={href} class="internal internal-link tag-link note-properties-chip">
          {tag}
        </a>
      );
    }
